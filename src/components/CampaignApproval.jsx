...
function CampaignApproval({ campaigns, setCampaigns }) {
  ...
  async function reject(id) {
    ...
    const campaignRef = doc(db, 'campaigns', id);
    await updateDoc(campaignRef, { status: 'rejected', rejectionReason: rejectReason });
        ...
        campaigns.filter(c => c.status === 'pending').map(c => (
          <div key={c.id} className="flex items-center p-3 border-b">
            ...
            <div className="flex space-x-2">
              <button onClick={() => approve(c.id)} className="text-green-600 hover:text-green-800">
                <FaCheck />
              </button>
              <button onClick={() => setSelectedCampaignId(c.id)} className="text-red-600 hover:text-red-800">
                <FaTimes />
              </button>
              {selectedCampaignId === c.id && (
                <div className="flex items-center mt-2">
                  ...
                  />
                  <button onClick={() => reject(c.id)} className="bg-red-600 text-white px-2 py-1 rounded-md">
                    Confirm
                  </button>
                </div>
              )}
