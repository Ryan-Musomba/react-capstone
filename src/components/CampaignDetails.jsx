...
function CampaignDetails() {
  ...
  async function handleDonation(e) {
    ...
    if (campaignData.status !== 'approved') {
      setError('This campaign is not approved for donations.');
      setDonationLoading(false);
      return;
    }
    ...
    if (amount > remaining) {
      setError(`Donation amount exceeds remaining goal of $${remaining.toFixed(2)}.`);
      setDonationLoading(false);
      return;
    }
    ...
    await Promise.all([
      ...
      updateDoc(campaignRef, { amountRaised: currentRaised + amount }),
    ]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-2 bg-white p-6 rounded-lg shadow-md">
            ...
            <p className="text-sm text-gray-500 mb-4">Created by: {campaign.creatorName || 'Unknown'}</p>
            ...
            {isGoalReached && <p className="text-green-600 font-semibold mb-4">Goal Reached! No more donations needed.</p>}
            {isExpired && !isGoalReached && <p className="text-red-600 font-semibold mb-4">This campaign has expired.</p>}
            <p className="text-sm text-gray-500 mb-4">Deadline: {new Date(campaign.deadline).toLocaleDateString()}</p>
            <p className="text-sm text-gray-500">Category: {campaign.category} | Urgency: {campaign.urgency}</p>
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            {isGoalReached || isExpired ? (
              <p className="text-gray-600">{isGoalReached ? 'This campaign has reached its goal.' : 'This campaign has expired.'}</p>
            ) : (
              <form onSubmit={handleDonation}>
                {!currentUser && (
                  <div className="mb-4 p-3 bg-yellow-100 text-yellow-800 rounded-md">
                    Please <button type="button" onClick={() => navigate('/?login=true')} className="text-indigo-600 hover:underline">login</button> to donate
                  </div>
                )}
                <div className="mb-4">
                  ...
                  <div className="flex space-x-2 mb-2">
                    {predefinedAmounts.map(amount => (
                      <button
                        ...
                        className={`px-4 py-2 border rounded-md ${donationAmount === amount.toString() ? 'bg-indigo-600 text-white' : 'bg-gray-200'}`}
                  ...
                  {donationAmount && campaign?.fundingGoal && parseFloat(donationAmount) > (campaign.fundingGoal - (campaign.amountRaised || 0)) && (
                    <p className="text-yellow-600 text-sm mt-1">
                      Amount exceeds remaining goal of ${(campaign.fundingGoal - (campaign.amountRaised || 0)).toFixed(2)}.
                    </p>
                  )}
                </div>
                <div className="mb-4">
