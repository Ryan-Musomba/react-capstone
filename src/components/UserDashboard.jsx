...
import { FaDonate, FaAd, FaArrowLeft, FaMoneyBillWave } from 'react-icons/fa';
...
function UserDashboard() {
  ...
  useEffect(() => {
    ...
    const myDonationsQuery = query(collection(db, 'donations'), where('donorId', '==', currentUser.uid));
    const unsubscribeMyDonations = onSnapshot(myDonationsQuery, snapshot => {
      const myDonationsList = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
      }));
  ...
  async function handleCampaignSubmit(e) {
    ...
    setNewCampaign({ name: '', description: '', fundingGoal: '', deadline: '', category: '', location: '', urgency: 'low', imageUrl: '' });
  ...
  const currentMyDonations = myDonations.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const totalMyDonationPages = Math.ceil(myDonations.length / itemsPerPage);
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="flex h-[calc(100vh-64px)]">
        <div className="w-64 bg-white shadow-md">
          <div className="p-4">
            <h2 className="text-2xl font-bold text-indigo-600">User Dashboard</h2>
          </div>
          <div className="mt-4 space-y-2">
            <SidebarItem icon={<FaAd />} text="My Campaigns" active={view === 'campaigns'} onClick={() => setView('campaigns')} />
            <SidebarItem icon={<FaDonate />} text="Campaign Donations" active={view === 'donations'} onClick={() => setView('donations')} />
            <SidebarItem icon={<FaMoneyBillWave />} text="My Donations" active={view === 'myDonations'} onClick={() => setView('myDonations')} />
            <SidebarItem icon={<FaAd />} text="Create Campaign" active={view === 'create'} onClick={() => setView('create')} />
          </div>
        </div>
                    ...
                    {currentCampaigns.map(c => (
                      <div key={c.id} className="p-4 border rounded-lg">
                        ...
                        <p className="text-sm text-gray-500">
                          Goal: ${c.fundingGoal.toFixed(2)} | Raised: ${(c.amountRaised || 0).toFixed(2)} | Needed: ${(c.fundingGoal - (c.amountRaised || 0)).toFixed(2)}
                        </p>
                        {c.amountRaised >= c.fundingGoal && <p className="text-green-600 font-semibold">Goal Reached!</p>}
                        ...
                        <p className="text-sm text-gray-500">Deadline: {new Date(c.deadline).toLocaleDateString()}</p>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              {myDonations.length === 0 ? (
                ...
                <>
                  ...
                  <div className="mt-4 flex justify-center space-x-2">
                    {Array.from({ length: totalMyDonationPages }, (_, i) => (
                      <button
                        key={i + 1}
                        onClick={() => setCurrentPage(i + 1)}
                        className={`px-3 py-1 rounded-md ${currentPage === i + 1 ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-600'}`}
